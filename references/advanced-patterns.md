# Advanced Reference: Component Patterns & Code Templates / 高级参考：组件模式与代码模板

This file contains advanced patterns and code templates to reference when implementing specific tasks.

> 本文件收录了一组高级模式与代码模板，用于在实现特定任务时快速引用与复用。

## Table of Contents / 目录

1. [Responsive Slide Engine / 响应式幻灯片引擎](#responsive-slide-engine)
2. [Device Simulation Frames / 设备模拟框](#device-simulation-frames)
3. [Tweaks Panel Implementation / Tweaks 面板实现](#tweaks-panel-implementation)
4. [Animation Timeline Engine / 动画时间轴引擎](#animation-timeline-engine)
5. [Design Canvas (Multi-option Comparison) / 设计画布（多方案对比）](#design-canvas)
6. [Dark Mode Toggle / 暗黑模式切换](#dark-mode-toggle)
7. [Data Visualization Templates / 数据可视化模板](#data-visualization-templates)

---

## Responsive Slide Engine / 响应式幻灯片引擎

For building fixed-size presentations that auto-fit to any viewport.

> 用于构建固定画布（例如 1920×1080）的演示稿，并自动缩放适配任意视口。
...
> **关键约定**：
> - 内部数组可用 0 索引，**但展示给用户的页码必须从 1 开始**
> - 每页 `<section class="slide">` 都要有 `data-screen-label="01 Title"`、`data-screen-label="02 Agenda"` 等标签，便于定位与沟通
> - 控制按钮放在缩放容器 `.stage` **外部**，保证小屏仍可点击
...
## Device Simulation Frames / 设备模拟框

### iPhone Frame / iPhone 设备框
...
## Tweaks Panel Implementation / Tweaks 面板实现
...
## Animation Timeline Engine / 动画时间轴引擎
...
### Chart.js Quick Start / Chart.js 快速开始
...
## Color System Best Practices / 配色系统最佳实践

Use oklch to define a harmonious color system:

> 使用 oklch 定义更和谐、可推导的颜色系统。
...
## Font Recommendations (Non-default Choices) / 字体建议（默认字体之外）
...
> ⚠️ **以下是经验建议，并非硬性规则。**
> - 优先使用品牌/设计系统已指定的字体；只有当用户没有提供字体体系时，才参考此表。
> - 唯一硬规则：**避免 Inter / Roboto / Arial / Fraunces / system-ui**（AI 输出过度使用，容易“一眼 AI”）。
> - 选字体看“气质匹配”，而不是“流行”。此表仅列常见高质量选择，并不穷举。
...
## Color × Font Pairing Reference / 配色 × 字体搭配参考
...
> ⚠️ **以下是经验搭配建议，并非硬规则。** 当你**完全没有设计上下文**时，可以先选一个作为起点——总比 Inter + #3b82f6 强。
> 一旦用户提供了品牌/设计系统/参考站点，请立刻停止依赖此表，改为跟随用户材料。
